...
submission_dataset = pd.read_csv(test_file_path).values.reshape((-1, image_size, image_size, num_channels)).astype(np.float32)
...
del X_train,X_validation,X_test,y_train,y_validation,y_test,labels, data, dataset
...
x_valid = np.pad(valid_dataset, ((0, 0), (2, 2), (2, 2),(0, 0)), 'constant')
...
with tf.Session() as sess:
    ...
    for i in range(EPOCHS):
        ...
        for offset in range(0, num_examples, BATCH_SIZE):
            ...
            batch_x, batch_y = x_train[offset:end], train_labels[offset:end]
